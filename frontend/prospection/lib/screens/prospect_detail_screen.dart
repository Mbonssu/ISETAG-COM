...
class _DetailState extends State<ProspectDetailScreen>
                  ...
                  child: Row(children: [
                    ...
                    Expanded(
                      ...
                      children: [
                        // Nom + badge
                        Row(children: [
                          Text(prospect.prosp.nomComplet, //'Marie L.',
                              style: const TextStyle(
                                  fontSize: 17,
                                  fontWeight: FontWeight.w800,
                                  color: Colors.white)),
                        ...
                        _contactRow(Icons.phone_outlined,
                            prospect.prosp.telephone), //'+237 699 12 34 56'),
                        const SizedBox(height: 3),
                        _contactRow(
                            Icons.email_outlined,
                            prospect.prosp.email ??
                                'Aucun'), //'marie.l@email.com'),
                      ],
                    )),
                  ]),
                ),
  ...
  Widget _buildInfoTab() {
    final rows = [
      ...
      (Icons.school_outlined, 'Établissement', prospect.etablissement, false),
      (Icons.location_on_outlined, 'Adresse', prospect.prosp.adresse, false),
      (Icons.people_outline, 'Concerné', prospect.prosp.typeProspect, false),
      (Icons.man, 'sexe', prospect.prosp.sexe, false),
      (
        Icons.edit_outlined,
        'Commentaire',
        prospect.prosp.commentaireGen,
        false
      ),
      (
        Icons.calendar_today_outlined, 'Date de relance proposée',
        //'25 Mai 2025',
        prospect.prosp.date_relance ?? 'Aucune date progammer',
        true
      ),
      (
        prospect.prosp.syncState.name == "pending"
            ? Icons.sync_disabled
            : Icons.sync,
        'Etat de synchronisation avec le serveur',
        //'25 Mai 2025',
        prospect.prosp.syncState.name,
        true
      ),
    ];
  ...
  /*Widget _buildFiliereTab() {
    return ListView.builder(
        padding: const EdgeInsets.all(14),
        itemCount:prospect.specialities.length ,
        itemBuilder: (_, i) => _filiereCard(
            prospect.specialities[i].libelleSpecialite,
            prospect.prosp.niveauEtude,
            prospect.specialities[i].commentaire)
  ...
  */
  ...
  Widget _buildFiliereTab() {
    final specs = prospect.specialities; // récup une fois

    if (specs.isEmpty) {
      return const Center(child: Text('Aucune spécialité'));
    }
    const SizedBox(height: 10);
    return ListView.builder(
      padding: const EdgeInsets.all(14),
      itemCount: specs.length, // CRUCIAL ← ajoute ça
      itemBuilder: (_, i) {
        final spec = specs[i];
        return _filiereCard(
        ...
        return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Column(children: [
            Container(
                ...
                height: 12,
                decoration: const BoxDecoration(
                    color: G.green, shape: BoxShape.circle)),
            if (!isLast)
              Container(
                  width: 2,
                  height: 62,
