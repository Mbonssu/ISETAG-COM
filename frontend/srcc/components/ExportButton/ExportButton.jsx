...
const ExportButton = ({ data, filename, title, filters, columns }) => {
  ...
  const handleExport = async (format) => {
    ...
    switch(format) {
      case 'excel':
        exportToExcel(data, filename, title, filters);
        break;
      case 'csv':
        exportToCSV(data, filename, title, filters);
        break;
      case 'json':
        exportToJSON(data, filename, title, filters);
        break;
      case 'pdf':
        await exportToPDF(data, filename, title, filters, columns);
        break;
      default:
        exportToExcel(data, filename, title, filters);
    }
