...
// de liaison "interetSpecialite" (idProspect + idSpecialite + niveauInteret).
//
// `interets` vient de interetService.getAll() : chaque élément a
// `specialite_details.libeleSpecialite` (fourni directement par le backend)
// et `niveauInteret`.
const NIVEAU_SCORE = {
  Faible: 1,
  Moyen: 2,
...
const scoreToLabel = (avgScore) => {
  const rounded = Math.round(avgScore);
  const label = Object.keys(NIVEAU_SCORE).find((k) => NIVEAU_SCORE[k] === rounded);
  return label || 'Moyen';
};

export function computeFiliereDistribution(interets) {
  const groups = {};

  interets.forEach((interet) => {
    const nom = interet.specialite_details?.libeleSpecialite
      || interet.specialite_details?.libele
      || interet.idSpecialite
      || 'Non renseignée';

    if (!groups[nom]) {
      groups[nom] = { name: nom, count: 0, scoreSum: 0, prospectIds: new Set() };
    }
    groups[nom].count += 1;
    groups[nom].scoreSum += NIVEAU_SCORE[interet.niveauInteret] || 1;
    if (interet.idProspect) groups[nom].prospectIds.add(interet.idProspect);
  });

  // Pondération : une filière choisie par beaucoup de monde avec un fort
  // niveau d'intérêt doit ressortir devant une filière juste "cochée" par
  // beaucoup de monde avec un intérêt faible. On classe donc par score
  // pondéré cumulé, pas par simple nombre de prospects.
  const totalScore = Object.values(groups).reduce((s, g) => s + g.scoreSum, 0) || 1;

  return Object.values(groups)
    .map((g) => ({
      name: g.name,
      value: g.prospectIds.size,
      percentage: Math.round((g.scoreSum / totalScore) * 100),
      avgNiveauLabel: scoreToLabel(g.scoreSum / g.count),
    }))
