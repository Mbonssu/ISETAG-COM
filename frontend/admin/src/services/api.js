...
export const apiRequest = async (url, options = {}) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      //  AJOUTÉ : clé de traduction attachée à l'erreur, indépendante du
      // message technique brut. Les composants appelants utilisent
      // getErrorMessage(error, t) (utils/errorMessages.js) pour afficher
      // un message propre et localisé plutôt que ce texte serveur.
      err.errorKey =
        response.status === 400 ? 'erreur400' :
        response.status === 401 ? 'erreur401' :
        response.status === 403 ? 'erreur403' :
        response.status === 404 ? 'erreur404' :
        response.status === 409 ? 'erreur409' :
        response.status >= 500 ? 'erreur500' :
        'erreurInconnue';
    ...
    //  AJOUTÉ : si l'erreur n'a pas déjà une errorKey (cas ci-dessus,
    // erreur HTTP avec statut connu), c'est que fetch() lui-même a échoué
    // AVANT même d'atteindre le serveur (serveur éteint, pas de réseau,
    // CORS bloqué, DNS...). Le navigateur lève alors un TypeError générique
    // ("Failed to fetch" / "NetworkError..."), illisible pour l'utilisateur.
    if (!error.errorKey) {
      error.errorKey = 'erreurReseau';
    }
...
//----------------------------utilisation de la bascule entre ngrok et l'adresse locale-----------------------------------------//
...
// //  CORRIGÉ : URL de base autrefois codée en dur (une seule adresse,
// // à décommenter manuellement selon l'environnement). Remplacée par une
// // résolution dynamique avec bascule automatique ngrok -> local, voir
// // services/apiBaseUrl.js. Les adresses elles-mêmes viennent du fichier
// // .env (REACT_APP_NGROK_URL / REACT_APP_LOCAL_URL), jamais du code.
// import { resolveApiBaseUrl, invalidateBaseUrlCache } from './apiBaseUrl';
...
//      * })
//      */
//     const finalUrl = await getApiUrl(url);
//     const response = await fetch(finalUrl, config);
...
//     if (!error.errorKey) {
//       error.errorKey = 'erreurReseau';
//       //  AJOUTÉ : si c'est une vraie erreur réseau (pas une erreur HTTP
//       // avec statut connu), l'adresse actuellement en cache (ngrok ou
//       // local) vient peut-être de tomber en cours de session. On invalide
//       // le cache pour que la PROCHAINE requête re-teste ngrok en priorité
//       // et bascule sur le local si besoin, sans attendre les 5 minutes
