...
export const agentService = {
  ...
  getAll: (params = {}) => {
    ...
    ('📡 GET all agents (via users):', queryString);
   ...
   */
  getById: (idAgent) => {
    ('📡 GET agent by ID (via users):', idAgent);
  ...
  create: (data) => {
    ...
    ('📝 CREATE agent (via users):', payload);
   ...
   */
  update: (idAgent, data) => {
    ('📝 UPDATE agent (via users):', idAgent, data);
   ...
   */
  delete: (idAgent) => {
    ('🗑️ DELETE agent (via users):', idAgent);
